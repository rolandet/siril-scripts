...
"""
Multi-Night Stacking for Siril 1.4 (PyQt6) — with Siril console integration (sirilpy) - version 2.0
...
class Project:
    ...
    mosaic_overlap_norm: bool = True               # requires -maximize in Siril
    mosaic_feather_px: int = 50
    link_feather_to_overlap: bool = False

    # Drizzle switches scoped to where to apply it (UI-only for now)
    mosaic_drizzle_per_panel: bool = False
    mosaic_drizzle_mosaic: bool = False
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "mosaic_drizzle_mosaic": self.mosaic_drizzle_mosaic,
    ...
    def from_dict(d: Dict) -> "Project":
        ...
        p.mosaic_drizzle_mosaic   = bool(d.get("mosaic_drizzle_mosaic", False))
    ...
    def _build_mosaic_phase1(self) -> str:
        ...
        for sess in p.sessions:
            ...
            for panel in panels:
                ...
                # --- Register the chosen sequence (with WCS undistortion); drizzle support unchanged ---
                reg_flags = " -2pass" if p.two_pass else ""
                if p.drizzle_enabled:
                    drizzle_args = (
                    ...
                    L.append(f"# Register panel {pid} sequence with WCS undistortion + drizzle")
                    L.append(f"register {seq_base} -disto=file platesolve_data.wcs{reg_flags}{drizzle_args}")
                    L.append(f"seqapplyreg {seq_base}{drizzle_args}")
                else:
                    L.append(f"# Register panel {pid} sequence with WCS undistortion")
                    L.append(f"register {seq_base} -disto=file platesolve_data.wcs{reg_flags}")
                    if p.two_pass:
        ...
        for pid, seqs in panel_seq_map.items():
            ...
            if len(seqs) == 1:
                ...
                cmd = ["stack", seq_name] + parts + ["-norm=addscale", "-maximize", "-rgb_equal", "-output_norm"]
                ...
                out_name = f"{safe_slug(pid)}_final"
                cmd = ["stack", f"r_{merged_name}"] + parts + ["-norm=addscale", "-maximize", "-rgb_equal", "-output_norm"]
                if bool(getattr(p, "stack_32bit", False)):
                    cmd.append("-32b")
                cmd.append(f"-out={out_name}")
...
def emit_phase2_mosaic(
    *,
    ...
    if len(finals) == 1:
        ...
        set_comp_if_needed(L, comp_state, 0)
    ...
    # Good defaults for mosaics: keep full canvas
...
class ProjectWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.chk_mosaic_overlap_norm = QtWidgets.QCheckBox("Normalize on Overlaps (requires Maximize framing)")
        self.chk_mosaic_overlap_norm.setToolTip(
            ...
            "Requires 'Maximize' framing in stacking so that overlap regions are preserved.\n"
            "Useful for evening out background/brightness differences between panels."
        )        
        mosaic_form.addWidget(self.chk_mosaic_bg, 5, 0, 1, 2)
        mosaic_form.addWidget(self.chk_mosaic_overlap_norm, 5, 2, 1, 2)
        ...
        self.chk_mosaic_drizzle_panel = QtWidgets.QCheckBox("Drizzle per panel (recommended)")
        self.chk_mosaic_drizzle_mosaic = QtWidgets.QCheckBox("Drizzle mosaic (advanced, huge files)")
        self._drizzle_guard = False
        self.chk_mosaic_drizzle_panel.toggled.connect(self._on_drizzle_scope_toggled)
        self.chk_mosaic_drizzle_mosaic.toggled.connect(self._on_drizzle_scope_toggled)        
        mosaic_form.addWidget(self.chk_mosaic_drizzle_panel, 7, 0, 1, 2)
        mosaic_form.addWidget(self.chk_mosaic_drizzle_mosaic, 7, 2, 1, 2)
        self.chk_mosaic_drizzle_panel.setToolTip(
            "Drizzle each panel during its own registration. Upsamples per panel; recommended to avoid giant mosaic files."
        )
        self.chk_mosaic_drizzle_mosaic.setToolTip(
            "Drizzle the stitched mosaic during mosaic registration. Creates very large files. Use only if you did NOT drizzle per panel."
        )
        ...
        mosaic_form.addWidget(self.chk_auto_grid, 8, 0, 1, 4)
        ...
        self.chk_mosaic_drizzle_mosaic.toggled.connect(self.mark_dirty)
    ...
    def _set_mosaic_controls_enabled(self, enabled: bool):
        widgets = [
            ...
            # normalization / blending
            self.chk_mosaic_bg, self.chk_mosaic_overlap_norm, self.sp_mosaic_feather,
            # drizzle scope
            self.chk_mosaic_drizzle_panel, self.chk_mosaic_drizzle_mosaic,
            # grid helpers
    ...
    def _on_mosaic_toggled(self, enabled: bool):
        ...
        self._on_drizzle_scope_toggled()
    ...
    def _on_drizzle_scope_toggled(self, *_):
        """Mutually exclusive behavior for the two mosaic drizzle checkboxes.
        Clicking one turns the other off. Both remain enabled when Mosaic mode is ON."""
        if getattr(self, "_drizzle_guard", False):
            return

        # Respect the Mosaic enable state: do nothing when it's OFF
        if not self.chk_mosaic_enabled.isChecked():
            return

        self._drizzle_guard = True
        try:
            src = self.sender() if hasattr(self, "sender") else None

            panel_on  = self.chk_mosaic_drizzle_panel.isChecked()
            mosaic_on = self.chk_mosaic_drizzle_mosaic.isChecked()

            # If both are ON, prefer per-panel (unless user just turned mosaic ON)
            if panel_on and mosaic_on:
                if src is self.chk_mosaic_drizzle_panel:
                    self.chk_mosaic_drizzle_mosaic.setChecked(False)
                elif src is self.chk_mosaic_drizzle_mosaic:
                    self.chk_mosaic_drizzle_panel.setChecked(False)
                else:
                    # On load with both true, default to per-panel
                    self.chk_mosaic_drizzle_mosaic.setChecked(False)

            # Do NOT touch .setEnabled() here; _set_mosaic_controls_enabled() governs that.
        finally:
            self._drizzle_guard = False
    ...
    def refresh_from_model(self):
        ...
        try:
            ...
            self.chk_mosaic_drizzle_mosaic.setChecked(p.mosaic_drizzle_mosaic)
            self._update_feather_from_overlap()            
    ...
    def push_to_model(self):
        ...
        p.mosaic_drizzle_mosaic = self.chk_mosaic_drizzle_mosaic.isChecked()
        # Safety: prefer per-panel if both are true (flip to prefer mosaic if you like)
        if p.mosaic_drizzle_per_panel and p.mosaic_drizzle_mosaic:
            p.mosaic_drizzle_mosaic = False        
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("OSC Multi-Night Stacking (Siril 1.4) Version 2.0")
        ...
        about_action.triggered.connect(lambda: QtWidgets.QMessageBox.information(
            self, "About",
            "OSC Multi-Night Stacking for Siril 1.4 (PyQt6) Version 2.0\n"
