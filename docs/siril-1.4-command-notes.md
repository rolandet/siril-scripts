...
- v3.0 `OIII Combine Policy` defaults to merging all extracted OIII subs before stacking. The advanced weighted modes stack Ha/OIII-derived OIII and SII/OIII-derived OIII separately, align them as `nb_oiii_blend` with `seqapplyreg nb_oiii_blend -framing=min`, match the SII/OIII-derived OIII master to the Ha/OIII-derived OIII master according to `NB Channel Balancing` (`Median/MAD Match`, `Background Match Only`, or no source matching for `None`), then `pm` blend the two sources using either generated sub-count weights or the manual slider weight.
