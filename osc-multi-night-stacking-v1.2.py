...
"""
Multi-Night Stacking for Siril 1.4 (PyQt6) — with Siril console integration (sirilpy) - version 1.2
...
2. **Prepare Working Directory (Symlink/copy Files)**
   ...
   - Each session will therefore have a `process/` subfolder holding the working data.
...
4. **Run Siril Script**
   ...
   - The application automatically executes the generated script via the Siril Python API when running inside Siril, or uses **`siril-cli`** as a backup when running externally.
   - Progress and script output appear in the Siril console and in a log file in the *working directory*.
   - When finished, your combined stack is saved automatically as **`[ProjectName]_final.fit`** and (when possible) loaded into Siril.
"""
...
class QuickStartDialog(QtWidgets.QDialog):
    ...
    def __init__(self, markdown_text: str, parent=None):
        ...
        font = self.font()
        font.setPointSizeF(font.pointSizeF() * 1.2)  # ~20% larger text
        self.setFont(font)
        self.setWindowTitle("Quick Start Instructions")
        self.setModal(True)
        self.setSizeGripEnabled(True)

        # Size tuned for laptop/1080p; scrollable for smaller screens
...
class Project:
    ...
    def from_dict(d: Dict) -> "Project":
        ...
        p.force_cli = d.get("force_cli", False)
...
def get_siril_version(siril_path: str) -> Optional[Tuple[int, int, int, str]]:
    """
    Return (major, minor, patch, raw_string) for siril-cli, or None on failure.

    raw_string is the full stdout from 'siril-cli -v', stripped, so we can compare
    it directly to the Siril GUI version from the environment.
    """
    try:
...
class SirilCommandBuilder:
    ...
    def build(self) -> str:
        ...
        L.append("")
        L.append("setfindstar reset")
        # No 'close' here — keep the final image open if Siril loads it
        # L.append("close")
        return "\n".join(L)
...
class ProjectWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Match v2.0 semantics: siril-cli is optional and only needed outside Siril
        ...
        self.cb_force_cli.toggled.connect(self._on_force_cli_toggled)
        ...
        sr.addWidget(self.cb_force_cli)
        ...
        bottom.addWidget(self.lbl_run_mode, 1)
    ...
    # ---------------- helpers ----------------
    def _on_force_cli_toggled(self, checked: bool):
        # Mark project dirty unless we're in a programmatic update
        if not self._suspend_dirty and not self._loading_session:
            self._dirty = True
            self.status_message.emit("Project has unsaved changes.")

        # Update the run-mode label hint
        try:
            if checked:
                self.lbl_run_mode.setText("Run mode: siril-cli (forced)")
                self.lbl_run_mode.setStyleSheet("color: #b71c1c;")  # reddish
            else:
                self.lbl_run_mode.setText("Run mode: not started")
                self.lbl_run_mode.setStyleSheet("color: #666666; font-style: italic;")
        except Exception:
            pass
    ...
    def _detect_siril_home_dir(self) -> str | None:
        ...
        def _parse_pwd(out: str) -> str | None:
            # Handle typical "Current working directory: '...'" output from Siril.
            for line in out.splitlines():
                line = line.strip()
                if not line:
                    continue
                if "Current working directory" in line:
                    # e.g. Current working directory: 'C:\Users\rolan\Documents\N.I.N.A\Targets\sh2-129'
                    m = re.search(r"['\"](.+?)['\"]", line)
                    if m:
                        return m.group(1)
                # Fallback: first non-empty line that looks like an absolute path
                if os.path.isabs(line):
                    return line
            return None

        # 1) Live Siril via sirilpy
        try:
            if getattr(self, "siril", None) is not None and getattr(self.siril, "iface", None) is not None:
                out = self.siril.iface.cmd("pwd")
                cand = _parse_pwd(out or "")
                if cand and os.path.isdir(cand):
                    return cand
        except Exception:
            pass

        # 2) siril-cli fallback
        try:
            siril_exe = find_siril_cli(getattr(self.project, "siril_cli_path", None))
            if siril_exe and os.path.exists(siril_exe):
                import tempfile
                import pathlib
                import subprocess
        ...
        # Log a note if everything failed (optional)
        try:
            if getattr(self, "siril", None) is not None and getattr(self.siril, "connected", False):
                self.siril.log("[Init] Default working dir guess: (none)")
        except Exception:
            pass
    ...
    def refresh_from_model(self):      
        ...
        try:
            ...
            # NEW: siril-cli path + Force CLI reflect the model
            self.ed_siril.setText(p.siril_cli_path or "")
            if hasattr(self, "cb_force_cli"):
    ...
    def run_siril(self):
        ...
        # Push current UI to model without changing dirty flag
        self.push_to_model()
        p = self.project
        if not p.working_dir:
        ...
        # Decide whether to use the Python API or siril-cli
        use_api = (
            not getattr(p, "force_cli", False)
            and getattr(self, "siril", None) is not None
            and getattr(self.siril, "iface", None) is not None
        )

        # --- PREFERRED: run inside Siril via sirilpy if available and not forcing CLI ---
        if use_api:
            try:
                # Determine project working directory and log the run
                proj_dir = Path(p.working_dir or "").resolve().as_posix()
                if self.siril.connected:
                    self.siril.log(f'[run] Executing SSF inside Siril: cd "{proj_dir}" ; @run_project.ssf')

                # Change to the project working directory, then run the script by name
                self.siril.iface.cmd(f'cd "{proj_dir}"')
                self.siril.iface.cmd('@run_project.ssf')

                # Try to auto-load the final image in Siril if it exists
                try:
                    proj_slug = safe_slug(self.project.name)
                ...
                # Preserve whatever the dirty state was
                self._dirty = was_dirty
                ...
                # If in-Siril execution failed, fall back to CLI
                if self.siril.connected:
                    self.siril.log(f"[run] In-Siril execution failed, will try siril-cli. Error: {e}")
                try:
        ...
        # --- FALLBACK: run via siril-cli (existing behaviour) ---
        ...
        # Version (for log)
        ver_info = get_siril_version(siril)
        if ver_info is not None:
            major, minor, patch, raw_cli = ver_info
            self._run_siril_verstr = raw_cli
        else:
            major = minor = patch = None
            raw_cli = ""
            self._run_siril_verstr = "unknown"
        ...
        try:
            ...
            self._proc = subprocess.Popen(
                ...
                stderr=subprocess.STDOUT,
                text=True,
                encoding="utf-8",       # match v2.0: safer for odd console bytes
                errors="replace",
        ...
        # Reader thread and progress / completion logic are unchanged
        ...
        def on_finished(rc: int):
            ...
            if rc == 0:
                ...
                QtWidgets.QMessageBox.information(
                    self, "Run Siril",
                    f"Processing finished in {elapsed_txt}.\n\nLog saved to:\n{log_path}"
                )
            else:
                QtWidgets.QMessageBox.critical(
                    self, "Run Siril",
                    f"siril-cli exited with code {rc} after {elapsed_txt}.\n\nLog:\n{log_path}"
                )
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OSC Multi-Night Stacking (Siril 1.4) Version 1.2")
        ...
        # Quick Start Instructions (F1)
        quickstart_action = QtGui.QAction("Quick Start Instructions", self)
        quickstart_action.setShortcut(QtGui.QKeySequence("F1"))
        quickstart_action.triggered.connect(self.show_quick_start)
        ...
        about_action.triggered.connect(lambda: QtWidgets.QMessageBox.information(
            self, "About",
            "OSC Multi-Night Stacking for Siril 1.4 (PyQt6) Version 1.2\n"
        ...
        # (Optional future auto-show hook; currently a no-op)
    ...
    def _should_autoshow_quickstart(self) -> bool:
        ...
        read a QSettings flag or env var.
        """
        return False
